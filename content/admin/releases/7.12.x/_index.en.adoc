...
{{% notice warning %}} Important: We have verified an issue with the UTF-8 repair tool marking valid email addresses as deleted.
As such, we would discourage users from using the 'Repair utf encoding' option on the Repair actions via the Admin Tools menu or related Robo CLI commands while we look to investigate and rectify the issue.
We appreciate the continued assistance from the community in identifying this issue and we are actively working on resolving it as our highest priority.
{{% /notice %}}

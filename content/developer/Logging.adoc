...
If you're really digging into what is in structured argument values, you might want to aim for a much 
higher value.
...
A boolean **true** (without quotes) will cause the stack trace to be followed with an array of values
describing the current request. 

This is ad-hoc information, like breadcrumbs, gathered in the initial stages of the request to the server, 
namely while setting up **SugarApplication**, **SugarController**, and **SugarView** objects. 
Some relevant global variables are also included.

A boolean **false** (without quotes), or simply ommitting this entry will turn the feature off.

Here's a sample output from this option, showing useful information that is hard to get from the stack trace,
but quite useful to "locate" where you are in a SuiteCRM server request:

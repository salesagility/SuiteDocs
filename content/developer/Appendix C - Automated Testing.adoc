...
PHPUnit test are related to a unit you are testing. The path of the tests is identical to the path of the unit inside the `tests/...` folder. Each Unit test class has a suffix `...Test` and should extends the `SuiteCRM\StateCheckerPHPUnitTestCaseAbstract` class. Test methods prefix is `test...`.

For e.g.: the `/path/to/Example.php` contains a `class Example` then the related test should be in the `tests/path/to/ExampleTest.php`: + 
...
Codeception also support unit test based on PHPUnit framework.
see more about Codeception unit tests:
...
Implementation of state safe functional and acceptance test.

*Example for a Functional and/or Acceptance tests*
...
Implementation of state checker Codeception tests, class StateCheckerPHPUnitTestCaseAbstract extends PHPUnit_Framework_TestCase and override setUp() and tearDown() methods so if you want to call these methods in the heritance test classes you should call parent::setUp() and parent::tearDown() in it.
...
Implementation of state checker Codeception tests, class StateCheckerUnitAbstract extends Codeception\Test\Unit and override _before() and _after() methods so if you want to call these methods in the heritance test classes you should call parent::_before() and parent::_after() in it.
...
Implementation of state checker Codeception tests, it is using _before() and _after() methods so if you want to call these methods in the heritance test classes you should call parent::_before() and parent::_after() in it.
...
See more at `class SuiteCRM\StateCheckerConfig`
 ...
 - Cofiguration of state checker tests
See at `SuiteCRM\StateCheckerConfig` class.


=== State safe tests
about `StateSaver` and `StateChecker`
...
*Keep clean the system global environment state, especially in tests.* +
If a test leaves some extra data in the database, file system or super globals etc. could change the behaviore of the other test processes. When you wrote a new test make sure it *state safe*, which means tests should not leaves any 'garbage' data in the test environment state such as database, file system, superglobals etc..
...
PHPUnit test have to `extends SuiteCRM\StateCheckerPHPUnitTestCaseAbstract` instead `PHPUnit_Framework_TestCase`.

Codeception Cests should extends `SuiteCRM\StateCheckerCestAbstract` class. 
(note: don't forget to call parent::_before() and parent::_after() if you want to override these methods in your tests)


The following classes helps you to write state-safe code (tests), of course some of these classes you can use in any case when you have to change and restore anything in the global system state:
...
See more at the `SuiteCRM\StateChecker` class.
...
// create a new instane of StateChecker:
...
// here your database table 'stufftable' restored.
...
See more about the `SuiteCRM\StateChecker` configuration at the `SuiteCRM\StateCheckerConfig` class.
...
SuiteCRM\StateCheckerConfig configuration options have default values and each available in + 
`$sugar_config['state_checker'][$key]`.
 
Each configuration value available with a getter method: + 
...
SuperGlobals Collection determines which super globals are stored and restored.
...
(For more memory expensive task, for e.g collection stack trace information when `$saveTraces` is ON, + 
...
Store more information about hash-mismatch, which part having state of globals/filesys/database. +
(Slow working but give more information about the error location, use in development only)
...
Enum specified that tests needs to check system state, for Test Cases behaviour, possible values: +

`SuiteCRM\StateCheckerConfig::RUN_NEVER`: State check and save never run. +

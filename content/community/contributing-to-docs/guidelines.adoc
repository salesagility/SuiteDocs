...
with Hugo (our rendering engine), Learn (our Hugo Theme), and Asciidoc (the mark up style we use).

The explanations below are simply a short list of some frequently used syntax, and a few recommendations
...
Basically we are trying to emulate the way Hugo produces the final content under `public` directory, so that there
...
the page belongs. We use increments of 10 for `weight`, so that if we want to insert a new page inbetween 

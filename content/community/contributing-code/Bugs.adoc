...
{{%notice info%}}If the bug you're fixing is present in hotfix-7.8.x then you will need to make two seperate pull requests, one to hotfix-7.8.x and one to hotfix-7.10.x. The hotfix-7.10.x branch will be merged into hotfix and the develop branch so no more duplicates are necessary.`.{{%/notice%}}

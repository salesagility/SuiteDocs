...
Select the campaign you wish to create: *Newsletter*, *Email*, link:../19surveys/[Survey] (*SuiteCRM 7.10* onwards) or *Non-email based Campaign*. Non-email based campaigns allow you to keep a record of any campaigns that are run outwith *SuiteCRM* (e.g. a telesales or radio based campaign).
...
Click btn:[DELETE TEST ENTRIES] to remove any entries on the campaign status page that have been generated during testing with a 'test' type link:../14target-lists/[target list]. 

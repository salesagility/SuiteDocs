...
Other outbound accounts can be configured in addition to the default account for specific purposes, such as sending link:../../core-modules/15campaigns[Campaigns]. These can be set up from the Admin panel, under outbound accounts. If no other outbound accounts are configured, the default outgoing mail server account will be used.
...
A Bounce Handling account is used with link:../../core-modules/15campaigns[Campaigns] to handle bounced mail notifications when emails are undelivered. You will need to set up a Bounce Handling account in order to send a Campaign.
...
Setting up a personal email account in *SuiteCRM* allows the user to view personal mail accounts within the *Emails* module. Emails from personal mailboxes are not stored in the *SuiteCRM* database unless manually link:../../core-modules/8emails/#_import_mail[imported]. 
...
* *Email Editor* - This allows you to set the editor used when creating and editing link:../../core-modules/8emails#_email_templates[email templates] and also within the link:../../core-modules/15campaigns[Campaigns] module. Note that the *Email Editor* setting does not affect the *SuiteCRM Emails* module *Compose* view. This setting is not currently user-definable. 
...
Check this box to set up *SuiteCRM* to create a link:../../core-modules/16cases[Case] record from an incoming email. 
...
If *SuiteCRM* has been configured to auto-create cases, you can select or create an link:../../core-modules/8emails/#_email_templates[email template] to use as an automated reponse to notify the sender that a case has been created. If no template is specified here, this automated reponse will not be sent. image:286EmailsNewCaseAutoReply.png[New Case Auto-Reply template]
...
A Bounce Handling Account is used to manage bounce notifications for an email link:../../core-modules/15campaigns[campaign]. Bounced email addresses are recorded in the campaign status.

Once created, the bounce handling account can be selected by users when setting up a link:../../core-modules/15campaigns[campaign].
...
Configure the following additional settings for link:../../core-modules/15campaigns[Campaigns] here:
